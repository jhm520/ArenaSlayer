...
bool SShooterScoreboardWidget::IsPlayerSelectedAndValid() const
{
	...
	if( !SelectedPlayer.IsValid() )
	{
		...
		{
			const TSharedPtr<FUniqueNetId> OwnerNetId = PCOwner->PlayerState->UniqueId.GetUniqueNetId();
			return OwnerNetId.IsValid();
		}
	}
	else if( const AShooterPlayerState* PlayerState = GetSortedPlayerState(SelectedPlayer) ) 
	{
		const TSharedPtr<FUniqueNetId>& PlayerId = PlayerState->UniqueId.GetUniqueNetId();
...
bool SShooterScoreboardWidget::ProfileUIOpened() const
{
	if( IsPlayerSelectedAndValid() )
	{
		...
		const TSharedPtr<FUniqueNetId>& PlayerId = ( !SelectedPlayer.IsValid() ? OwnerNetId : GetSortedPlayerState(SelectedPlayer)->UniqueId.GetUniqueNetId() );
...
FString SShooterScoreboardWidget::GetPlayerName(const FTeamPlayer TeamPlayer) const
{
...
FString SShooterScoreboardWidget::GetStat(FOnGetPlayerStateAttribute Getter, const FTeamPlayer TeamPlayer) const
{
	int32 StatTotal = 0;
