...
void SShooterSplitScreenLobby::ConditionallyReadyPlayer( const int ControllerId, const bool bCanShowUI )
{
	...
	TSharedPtr< FUniqueNetId > UniqueNetId = GetGameInstance()->GetUniqueNetIdFromControllerId( ControllerId );
	...
	if(OnlineSub)
	{
		...
		if(IdentityInterface.IsValid())
		{
			...
			for ( int i = 0; i < NumActiveSlots; ++i )
			{
				...
				TSharedPtr<FUniqueNetId> Sponsor = IdentityInterface->GetSponsorUniquePlayerId( LocalPlayer->GetControllerId() );
...
void SShooterSplitScreenLobby::HandleLoginUIClosedAndReady( TSharedPtr<FUniqueNetId> UniqueId, const int UserIndex )
{
