...
void SShooterMenuWidget::Construct(const FArguments& InArgs)
{
	...
	const FString PlayerName = PlayerOwner.IsValid() ? PlayerOwner->GetNickname() : FString();
...
void SShooterMenuWidget::HandleProfileUISwapClosed(TSharedPtr<FUniqueNetId> UniqueId, const int ControllerIndex)
{
	UShooterGameInstance * GameInstance = PlayerOwner.IsValid() ? Cast< UShooterGameInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
