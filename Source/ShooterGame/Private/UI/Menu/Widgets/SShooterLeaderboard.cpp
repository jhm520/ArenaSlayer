...
bool SShooterLeaderboard::ProfileUIOpened() const
{
	if( IsPlayerSelectedAndValid() )
	{
		...
		const TSharedPtr<FUniqueNetId> OwnerNetId = PlayerOwner->GetPreferredUniqueNetId();
		check( OwnerNetId.IsValid() );

		const TSharedPtr<FUniqueNetId>& PlayerId = SelectedItem->PlayerId;
