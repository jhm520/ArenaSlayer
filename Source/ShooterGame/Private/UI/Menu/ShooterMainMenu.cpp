...
void FShooterMainMenu::BeginQuickMatchSearch()
{
	...
	Sessions->OnMatchmakingCompleteDelegates.Remove(OnMatchmakingCompleteDelegateHandle);
	OnMatchmakingCompleteDelegateHandle = Sessions->OnMatchmakingCompleteDelegates.AddSP(this, &FShooterMainMenu::OnMatchmakingComplete);
	if (!Sessions->StartMatchmaking(GetPlayerOwnerControllerId(), GameSessionName, SessionSettings, QuickMatchSearchSettingsRef))
	{
...
{
	// Add local users to the party
	if (GameInstance.IsValid())
	{
		GameInstance->AddPlayersToParty();
	}
...
void FShooterMainMenu::HelperQuickMatchSearchingUICancel(bool bShouldRemoveSession)
{
	...
	if (bShouldRemoveSession && Sessions.IsValid())
	{
		if (PlayerOwner.IsValid() && PlayerOwner->GetPreferredUniqueNetId().IsValid())
		{
			if (GameInstance.IsValid())
			{
				GameInstance->RemovePlayersFromParty();
			}
...
void FShooterMainMenu::OnMatchmakingComplete(FName SessionName, bool bWasSuccessful)
{
	...
	SessionInterface->OnMatchmakingCompleteDelegates.Remove(OnMatchmakingCompleteDelegateHandle);
...
static FString GetDemoName()
{
	...
	for ( int32 i = 0; i < MAX_DEMOS; i++ )
	{
		...
		const FString FullDemoName = DemoPath + DemoName + TEXT( ".demo" );
	...
	{
		FinalDemoName = FString( TEXT("?DemoRec=") ) + FinalDemoName;
	}
...
{	
	...
	{
		// Figure out a demo name if we are recording
		const FString DemoName = bIsRecordingDemo ? GetDemoName() : FString();

		FString const StartURL = FString::Printf(TEXT("/Game/Maps/%s?game=%s%s%s?%s=%d%s"), *GetMapName(), *GameType, GameInstance->GetIsOnline() ? TEXT("?listen") : TEXT(""), bIsLanMatch ? TEXT("?bIsLanMatch") : TEXT(""), *AShooterGameMode::GetBotsCountOptionName(), BotsCountOpt, DemoName.IsEmpty() ? TEXT(""): *DemoName );
...
{
	UShooterLocalPlayer* const SLP = Cast<UShooterLocalPlayer>(GetPlayerOwner());
