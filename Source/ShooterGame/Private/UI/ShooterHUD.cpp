...
//John
/** Draw context sensitive parts of the HUD*/
void AShooterHUD::DrawInteract()
{
...
void AShooterHUD::NotifyHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
...
void AShooterHUD::OnPlayerTalkingStateChanged(TSharedRef<FUniqueNetId> TalkingPlayerId, bool bIsTalking)
{
...
void AShooterHUD::AddChatLine(const FString& ChatString, bool bWantFocus)
{
	TryCreateChatWidget();
...
float AShooterHUD::ShowInfoItems(float YOffset, float ScaleUI, float TextScale)
{
	float Y = YOffset;
