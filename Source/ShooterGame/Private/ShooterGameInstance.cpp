...
void UShooterGameInstance::Init() 
{
	...
	SessionInterface->AddOnSessionUserInviteAcceptedDelegate_Handle( FOnSessionUserInviteAcceptedDelegate::CreateUObject( this, &UShooterGameInstance::HandleSessionUserInviteAccepted ) );
...
void UShooterGameInstance::HandleSessionUserInviteAccepted( 
...
void UShooterGameInstance::HandleNetworkConnectionStatusChanged( EOnlineServerConnectionStatus::Type ConnectionStatus )
{
...
TSharedPtr< FUniqueNetId > UShooterGameInstance::GetUniqueNetIdFromControllerId( const int ControllerId )
{
...
void UShooterGameInstance::AddPlayersToParty()
{
	auto PartyInterface = Online::GetPartyInterface();
	const auto FirstUniqueId = GetFirstGamePlayer() ? GetFirstGamePlayer()->GetPreferredUniqueNetId() : nullptr;
	if (GetIsOnline() && PartyInterface.IsValid() && FirstUniqueId.IsValid())
	{
		TArray<TSharedRef<FUniqueNetId>> SplitscreenPlayerIds;
		for (int i = 1; i < LocalPlayers.Num(); ++i)
		{
			auto SplitscreenUniqueId = LocalPlayers[i]->GetPreferredUniqueNetId();
			if (SplitscreenUniqueId.IsValid())
			{
				SplitscreenPlayerIds.Add(SplitscreenUniqueId.ToSharedRef());
			}
		}
		PartyInterface->AddLocalUsers(*FirstUniqueId, SplitscreenPlayerIds, IOnlineParty::FOnLocalUserOperationCompleteDelegate());
	}
...
void UShooterGameInstance::RemovePlayersFromParty()
{
	...
	if (GetIsOnline() && PartyInterface.IsValid())
	{
		...
		for (int i = 0; i < LocalPlayers.Num(); ++i)
		{
			auto PlayerUniqueId = LocalPlayers[i]->GetPreferredUniqueNetId();
			if (PlayerUniqueId.IsValid())
			{
				// Don't remove a player from the party they just accepted an invite to
...
void UShooterGameInstance::StartOnlinePrivilegeTask(const IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate& Delegate, EUserPrivileges::Type Privilege, TSharedPtr< FUniqueNetId > UserId)
{
